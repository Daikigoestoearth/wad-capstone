...
const createTaskSchema = Joi.object({
  ...
  status: Joi.string().valid(...VALID_STATUS).default('todo').messages({
    'any.only': `status harus salah satu dari: ${VALID_STATUS.join(', ')}.`
  }),
  priority: Joi.string().valid(...VALID_PRIORITY).default('medium').messages({
    'any.only': `priority harus salah satu dari: ${VALID_PRIORITY.join(', ')}.`
  }),
